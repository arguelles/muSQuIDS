...
int main()
{
  ...
  nusquids::marray<double,1> e_nodes = logspace(1.0e0*units.GeV,1.0e3*units.GeV,100);
  muSQUIDS musq(e_nodes);

  std::shared_ptr<ConstantDensity> constdens = std::make_shared<ConstantDensity>(5.0,0.5);
  std::shared_ptr<ConstantDensity::Track> constdens_track= std::make_shared<ConstantDensity::Track>(1000.*units.km);
  musq.Set_Body(constdens);
  musq.Set_Track(constdens_track);
